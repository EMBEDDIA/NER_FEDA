#I have modified this program to include BIOES support
# ==============================================================================
# https://raw.githubusercontent.com/epwalsh/pytorch-crf/master/pycrf/modules/crf.py
...
def allowed_transitions(constraint_type: str,
    ...
    if constraint_type == "BIOUL":
    ...
    elif constraint_type == "BIOES":
        for i, (from_bioes, *from_entity) in tokens.items():
            for j, (to_bioes, *to_entity) in tokens.items():

                if from_bioes in ["O", "B", "I", "S", "E"] and to_bioes in ["O", "B", "I", "S", "E"]:
                    is_allowed = any([
                        # O can transition to O, B-* or S-*
                        # E-x can transition to O, B-*, or S-*
                        # S-x can transition to O, B-*, or S-*
                        from_bioes in ('O', 'E', 'S') and to_bioes in ('O', 'B', 'S'),
                        # B-x can only transition to I-x or E-x
                        # I-x can only transition to I-x or E-x
                        from_bioes in ('B', 'I') and to_bioes in ('I', 'E') and from_entity == to_entity
                    ])
        ...
        # start transitions
        for i, (to_bioes, *to_entity) in tokens.items():
            if to_bioes in ('O', 'B', 'S'):
                allowed.append((start_tag, i))

        # end transitions
        for i, (from_bioes, *from_entity) in tokens.items():
            if from_bioes in ('O', 'E', 'S'):
                allowed.append((i, end_tag))

    elif constraint_type in ["BIO"]:
        for i, (from_bio, *from_entity) in tokens.items():
            for j, (to_bio, *to_entity) in tokens.items():

                if from_bio in ["O", "B", "I"] and to_bio in ["O", "B", "I"]:
                    is_allowed = any([
...
class ConditionalRandomField(torch.nn.Module):
    ...
    def forward(self,
        ...
        #Adrián
        #In the original version of the numerator and denominator were flipped
        log_numerator = self._input_likelihood(inputs, mask)
        log_denominator = self._joint_likelihood(inputs, tags, mask)
    ...
    def viterbi_tags(self,
        ...
        for prediction, prediction_mask in zip(logits, mask):
            ...
            #Adrián: We do not consider that the mask covers only the end of the prediction, it can be found at the begining too
            tag_sequence[1:(sequence_length + 1), :num_tags] = prediction[prediction_mask == 1]
            ...
            best_scores.append(viterbi_score.item())
